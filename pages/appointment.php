                            ...
                            <div class="appointment-form-step" id="step2">
                                ...
                                <div class="mb-6">
                                    <input type="date" id="appointment-date" name="appointment_date"
                                        ...
                                        min="<?= date('Y-m-d') ?>" />
                                    <!-- min ensures you can't pick past dates -->
                                </div>
    ...
    <script>
        ...
        function generateTimeSlots() {
            ...
            timeSlots.forEach(slot => {
                ...
                div.className = 'time-slot text-base cursor-pointer px-4 py-2 border rounded mb-2';
                div.textContent = slot.display_time;

                if (bookedSlots.includes(slot.time_value)) {
                    div.classList.add('booked', 'opacity-50', 'cursor-not-allowed');
                } else {
        ...
        appointmentForm.addEventListener('submit', function(e) {
            ...
            fetch('process-appointment.php', { // vérifie que le chemin est correct
                    method: 'POST',
                    body: formData
                })
                .then(res => res.json())
                ...
                <div class="text-center py-8">
                    ...
                    <h3 class="text-2xl font-bold text-gray-800 mb-3">${data.message}</h3>
        ...
        closeChat.addEventListener('click', () => chatboxWindow.classList.remove('active'));
        ...
        function addMessage(text, sender) {
            ...
            msg.innerHTML = `<div class="message-content">${text}</div><div class="message-time">${new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div>`;
        ...
        function getBotResponse(message) {
            ...
            if (lower.includes('appointment') || lower.includes('book')) return "You can book an appointment using the form above.";
            if (lower.includes('hour') || lower.includes('time')) return "Our office hours: Mon-Fri 9AM-5PM, Sat 9AM-1PM.";
            if (lower.includes('phone') || lower.includes('call')) return "You can call us at <?= $hero['phone_number'] ?>.";
            if (lower.includes('email') || lower.includes('contact')) return "Email us at LawfirmPrecision@outlook.com.";
            if (lower.includes('location') || lower.includes('address')) return "We're at: 7th floor, Astor Court, Port Louis.";
            return "For specific inquiries, please book an appointment with our firm.";
        }
